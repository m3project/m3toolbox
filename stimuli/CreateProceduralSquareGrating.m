% This function is a near-duplicate of CreateProceduralSineGrating.m that
% ships with PTB. The only difference is that it uses an alternative shader
% that produces a square wave grating instead of a sine grating.
%
% The two shader files for the square grating:
% - BasicSquareGratingShader.fraq.txt
% - BasicSquareGratingShader.vert.txt
% must exist in the same directory as this script.
%
% 18/10/2013 - Ghaith Tarawneh (ghaith.tarawneh@ncl.ac.uk)
%
function [gratingid, gratingrect] = CreateProceduralSquareGrating(windowPtr, width, height, backgroundColorOffset, radius, contrastPreMultiplicator)
...
shaderFile = fullfile(scriptPath, 'BasicSquareGratingShader');

if isinf(radius)
    
    % Load standard grating shader:
    
    %gratingShader = LoadGLSLProgramFromFiles([dir1 'BasicSquareGratingShader'], 1);
    
    gratingShader = LoadGLSLProgramFromFiles(shaderFile, 1);
    
else
    
    % Load grating shader with circular aperture support:
    
    %gratingShader = LoadGLSLProgramFromFiles({[dir1 './BasicSquareGratingShader.vert.txt'], 'ApertureSineGratingShader.frag.txt'}, 1);
    
    gratingShader = LoadGLSLProgramFromFiles({shaderFile, 'ApertureSineGratingShader.frag.txt'}, 1);
end
