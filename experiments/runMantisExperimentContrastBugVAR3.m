...
expt.name = 'Mantis Contrast Bug (VAR3)';
...
expt.addTags = {'VAR3'}; % add a tag here to indicate changes in condition
...
bugLum = [0 0.2 0.5]; % bug luminance conditions

bugUseNat = [0 1]; % 0 = uniform, 1 = natural (1/f)

backUseNat = [0 1]; % 0 = uniform, 1 = natural (1/f)
...
% filter out conditions with uniform 0.5 bug and uniform backgound
% (as the bug would be invisible)

k = (paramSet(:, 2) == 0.5) & (paramSet(:, 3) == 0) & (paramSet(:, 4) == 0);
...
args = struct('dir', paramSetRow(1), 'useNatBack', paramSetRow(4), ...
...
resultRow = runAfterTrial_VAR1(varargin);
